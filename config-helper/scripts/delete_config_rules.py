...
def main():
    ...
    # confirm with user
    print('\nDelete for profile_name=%s in target_region=%s: %d rules.' \
        % (profile_name, target_region, num_deployed_config_rule_names))
    while True:
    ...
    print("\nStarting deletion ...")
    success = config_util.delete_config_rules(profile_name, target_region, deployed_config_rule_names)
    if not success:
        print('delete_config_rules failed.  Exit.')
        return      
    print("\nDeletion completed.")
